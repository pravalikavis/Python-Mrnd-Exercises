...
5. Review the files lesson and write elegant code. Python api/features makes it possible to write concise and efficie
nt code.
'''
...
def sort_words(source, destination):
    ...
    """
    f=open(source,"r")
    l = [x.lower() if '#' not in x else x[:x.index('#')].strip() for x in f.readlines() if x[0] != '#' and x != '\n']
    l.sort()
    l = [x if x[-1]=='\n' else x+'\n'  for x in l if x != '']
