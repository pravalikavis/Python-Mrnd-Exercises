...
def left_binary_search(input, value):
    ...
    while(left <=right and mid < len(input)):

     if input[mid]==value:
        while(input[mid-1]==input[mid] and mid>=0):
            mid-=1
        if mid<0:
            mid+=1
        return mid
...
def test_left_binary_search_student():
    assert -1==left_binary_search([1,1],2)
    assert 1 == left_binary_search([1, 2], 2)
    assert 2 == left_binary_search(range(10), 2)
    assert 0 == left_binary_search([1, 1], 1)
    assert 2 == left_binary_search([1,2,3,3,4,4,5], 3)
    try:
        left_binary_search(1,0)
    except TypeError as te:
        print(te)

    assert None==  left_binary_search([1],None)
