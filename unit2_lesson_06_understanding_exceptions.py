...
# play with this method to understand the basic exception flow and understand how to follow a
#  stacktrace in the error log.
def test_exception_basic():
    print("starting test")
    #raise AssertionError("sample assertion error")  # this is equivalent to assert False,
    #  "sample assertion error"
    print("after raise")
    ...
    # again study what prints you got and the print of exception and make sure you understand the
        # control flow of an
    # an exception.
...
test_notes = """
In the following tests, to track the control flow, we use a 'result' list to mark the control
 flow points visited 
during execution. The contents of the 'result' at the end depend on which code paths were executed and
 which were not.
 
you must reason about the control flow and fill in the assert statement with the expected value of 
'result'.
"""
...
# Python 3 has implicitly tracks the inner exception and also allows you to replace one exception with
#  another.
