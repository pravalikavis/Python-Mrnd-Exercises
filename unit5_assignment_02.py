...
def generator_zip(seq1, seq2, *more_seqs):
    ...
    if more_seqs==():
       ...
       t=min(len(seq1),len(seq2),len_1(more_seqs) )
       if t>0:
        for x in range(t):
...
def test_generator_zip_student():
    gen = generator_zip(range(0),range(0))
    assert type(gen).__name__ == 'generator'
    check_generator(gen, 0, 0)
    gen = generator_zip(range(0), range(0),range(0),range(1))
    assert type(gen).__name__ == 'generator'
    check_generator(gen, 0, 0)
    try:
     gen = generator_zip(range(1), range(1), range(1),None)
    except TypeError as te:
        print(te)

    try:
        gen = generator_zip(None,None)
    except TypeError as te:
        print(te)
