...
def test_decorators_func_arguments():
    ...
    test_logs = ['Entered: add', 'Entered: sub', 'Entered: increment']

    assert 30 == add(10, 20)
    assert ['Entered: add', 'Entered: sub', 'Entered: increment','Entered: add' ] == test_logs
...
def test_decorators_arguments():
    ...
    @runif("MY_LINUX")
    def linux_test():
       return test_logs.append("linux_test called!")

    # run this only if MY_WINDOWS env variable is present
    @runif("MY_WINDOWS")
    def windows_test():
        return test_logs.append("windows_test called!")
...
def test_decorators_custom_wrap():
    ...
    def wraps(func):
        def newfunc(func1):
            def nf(*args,**kwargs):
                nf.func_name=func.__name__
                nf.func_doc=func.__doc__
